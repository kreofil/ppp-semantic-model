...
_Bool isCorrectOperandOfExit<Operand.Var* opd>() {
...
_Bool isCorrectConstantOfMinus<Constant* constant>(Operand** dst) {
  return 0;
}
// Проверка на целочисленную константу в команде Minus
_Bool isCorrectConstantOfMinus<Constant.Int* constant>(Operand** dst) {
...
_Bool isCorrectVarTypeOfMinus<Type* varType>(Operand** dst) {
  return 0;
}
// Проверка на целочисленный тип
_Bool isCorrectVarTypeOfMinus<Type.Int* varType>(Operand** dst) {
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Проверка операнда на допустимость. Обработчик обобщения
_Bool isCorrectOperandOfMinus<Operand* src>(Operand** dst) {
  // По умолчанию некорректный операнд
  // fprintf(stderr, "--- incorrect Operand Of Minus\n");
  return 0;
}
// Проверка операнда на допустимость. Целочисленная константа
_Bool isCorrectOperandOfMinus<Operand.Const* src>(Operand** dst) {
  Constant* constant = src->@;
  return isCorrectConstantOfMinus<constant>(dst);
}
// Проверка операнда на допустимость. Целочисленная переменная
_Bool isCorrectOperandOfMinus<Operand.Var* src>(Operand** dst) {
